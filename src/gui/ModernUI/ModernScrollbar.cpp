	...
	QWidget(_parent)
{
	...
	m_forwardButtonOpacity = 0;
	m_backwardButtonOpacity = 0;
...
void ModernScrollbar::setFollowValues(QVector<float> values)
{
	...
	m_forwardButtonOpacity = values[1];
	m_backwardButtonOpacity = values[2];
	update();
}
...
void ModernScrollbar::paintEvent(QPaintEvent *event)
{
	...
	if (m_isHorizontal)
	{
		forwardBtn = QRectF(QPointF(width() - height() + 1, 1), QSizeF(height() - 2, height() - 2));
		backwardBtn = QRectF(QPointF(1, 1), QSizeF(height() - 2, height() - 2));
	}
	else
	{
		forwardBtn = QRectF(QPointF(1, height() - width() + 1), QSizeF(width() - 2, width() - 2));
		backwardBtn = QRectF(QPointF(1, 1), QSizeF(width() - 2, width() - 2));
	}

	int shade = s_handleClickedShade;

	//what
	m_canvas.setBrush(QColor(shade, shade, shade, m_backwardButtonOpacity));
	m_canvas.drawRoundedRect(forwardBtn, 1, 1);

	m_canvas.setBrush(QColor(shade, shade, shade, m_forwardButtonOpacity));
	m_canvas.drawRoundedRect(backwardBtn, 1, 1);
...
void ModernScrollbar::mouseMoveEvent(QMouseEvent *event)
{
	...
	if (!(m_isInDragOperation || m_isInStartMoveOperation || m_isInEndMoveOperation)
			&& mousePosValue < 0)
		m_lazyFollower->updateTarget(1, 255);
	else
		m_lazyFollower->updateTarget(1, 0);

	if (!(m_isInDragOperation || m_isInStartMoveOperation || m_isInEndMoveOperation)
			&& mousePosValue > m_totalSize)
		m_lazyFollower->updateTarget(2, 255);
	else
		m_lazyFollower->updateTarget(2, 0);
